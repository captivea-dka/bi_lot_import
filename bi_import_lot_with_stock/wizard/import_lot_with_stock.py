...
class StockImport(models.TransientModel):
	...
	def import_file(self):
		...
		if self.import_option == 'xls':
			...
			inventory_id=inventory_obj.create({'name':self.name,'prefill_counted_quantity':'counted','company_id':res_user_id.company_id.id})
				...
				if row_no <= 0:
					...
					if line:
						if self.import_prod_option == 'name':
							...
							product_id = product_obj.search([('barcode','=',line[1])],limit=1)
							if not product_id:
						...
						if product_id:
							if self.lot_with_date == True:
								...
								for rec in range(4,8):
									if isinstance(line[rec],str):
										if line[rec].split('/'):
											if len(line[rec].split('-')) >1 or len(line[rec].split('/')[0]) >2 or len(line[rec].split(':')) <3:
												raise ValidationError(_('Wrong Date Format. Date Should be in format MM/DD/YYYY H:M:S.'))
								
								best_date= datetime.strptime(line[4], "%m/%d/%Y %H:%M:%S")- timedelta(hours=5,minutes=30)
								removal_date= datetime.strptime(line[5], "%m/%d/%Y %H:%M:%S")- timedelta(hours=5,minutes=30)
								exp_date= datetime.strptime(line[6], "%m/%d/%Y %H:%M:%S")- timedelta(hours=5,minutes=30)
								alert_date= datetime.strptime(line[7], "%m/%d/%Y %H:%M:%S")- timedelta(hours=5,minutes=30)
							...
							stock_line_obj.create({'product_id':product_id.id,'prod_lot_id':lot_id,'location_id':self.location.id,'product_qty':values['qty'],
			...
			keys = ['serial/lot number', 'product', 'qty','Internal ref', 'best before date', 'removal date','end of life date',
					'alert date']
							...
							if product_id:
								...
								if self.lot_with_date == True:
									...
									if isinstance(values['best before date'],str):
										...
										if date.split('/'):
											if len(date.split('-')) >1 or len(date.split('/')[0]) >2 or len(date.split(':')) <3:
												raise ValidationError(_('Wrong Date Format. Date Should be in format MM/DD/YYYY H:M:S.'))
									...
									best_date= datetime.strptime(values['best before date'], "%m/%d/%Y %H:%M:%S")- timedelta(hours=5,minutes=30)
									removal_date= datetime.strptime(values['removal date'], "%m/%d/%Y %H:%M:%S")- timedelta(hours=5,minutes=30)
									exp_date= datetime.strptime(values['end of life date'], "%m/%d/%Y %H:%M:%S")- timedelta(hours=5,minutes=30)
									alert_date= datetime.strptime(values['alert date'], "%m/%d/%Y %H:%M:%S")- timedelta(hours=5,minutes=30)	
...
class ProductionLotInherit(models.Model):
    ...
    qty_file = fields.Float('Quantity')
